...
def init_client():
    ...
    إذا فشلت 3 محاولات → يجلب بروكسي جديد ويعيد المحاولة (لا نهائي).
    """
    global client, PROXY_LIST

    print("🚀 بدء تهيئة الاتصال بـ Binance...")

    while True:  # 🔁 لا يتوقف أبداً حتى ينجح
        ...
        if not USE_PROXY or proxy:
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    ...
                    # اختبار الاتصال
                    client.ping()
                    print(f"✅ الاتصال بـ Binance ناجح! (بروكسي: {proxy['http'] if proxy else 'بدون بروكسي'})")
                    return True
        ...
        # 🔄 تبديل البروكسي - مسح القائمة وجلب جديدة
        print("🔄 جلب بروكسيات جديدة والمحاولة مرة أخرى...")
        PROXY_LIST = []  # مسح القائمة القديمة
        time.sleep(PROXY_ROTATION_DELAY)

# ================= إشعارات التليجرام =================

def send_telegram_message(message):
    """إرسال إشعار مع إعادة محاولة 3 مرات فقط"""
...
def git_commit_and_push():
    """رفع لـ GitHub مع إعادة محاولة 3 مرات فقط"""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
...
def get_prices():
    """جلب الأسعار مع إعادة محاولة 3 مرات فقط"""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
...
def execute_buy():
    """تنفيذ الشراء مع إعادة محاولة 3 مرات فقط"""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
...
def execute_sell(qty):
    """تنفيذ البيع مع إعادة محاولة 3 مرات فقط"""
    for attempt in range(1, MAX_RETRIES + 1):
        try:
    ...
    for pos in positions:
        ...
        if current_price >= min_sell:
            ...
            if order:
                ...
                # فشل البيع بعد 3 محاولات - احتفظ بالمركز
...
def main():
    ...
    # 🔁 إعادة محاولة الاتصال لا نهائية مع تبديل البروكسي
    print("🚀 بدء السكربت...")
    init_client()  # لا يتوقف أبداً حتى ينجح
    ...
    while time.time() < end_time:
        ...
        try:
            ...
            # إذا فشل جلب الأسعار - تخطى الدورة
            ...
            # إعادة تهيئة الاتصال إذا كان خطأ IP أو اتصال
            if "IP" in str(e) or "banned" in str(e).lower() or "connection" in str(e).lower():
                print("🔄 إعادة تهيئة الاتصال مع بروكسي جديد...")
                time.sleep(5)
                init_client()  # يجلب بروكسي جديد ولا يتوقف
